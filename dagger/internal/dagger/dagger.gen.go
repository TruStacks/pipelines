...
func (r *Go) Build(src *Directory, opts ...GoBuildOpts) *Directory {
...
func (r *Go) FromVersion(version string) *Go {
	q := r.query.Select("fromVersion")
...
func (r *Go) Generate(src *Directory) *Directory {
	assertNotNil("src", src)
...
func (r *Go) GolangCilint(src *Directory, opts ...GoGolangCilintOpts) *Container {
	assertNotNil("src", src)
	...
	for i := len(opts) - 1; i >= 0; i-- {
		...
		// `timeoutInSeconds` optional argument
		if !querybuilder.IsZeroValue(opts[i].TimeoutInSeconds) {
			q = q.Arg("timeoutInSeconds", opts[i].TimeoutInSeconds)
		}
	}
	q = q.Arg("src", src)
...
func (r *Go) Gotestsum(src *Directory, opts ...GoGotestsumOpts) *Container {
	assertNotNil("src", src)
	q := r.query.Select("gotestsum")
	for i := len(opts) - 1; i >= 0; i-- {
		// `packages` optional argument
		if !querybuilder.IsZeroValue(opts[i].Packages) {
			q = q.Arg("packages", opts[i].Packages)
		...
		if !querybuilder.IsZeroValue(opts[i].Nest) {
			q = q.Arg("nest", opts[i].Nest)
		}
...
func (r *Go) ModCache() *CacheVolume {
	q := r.query.Select("modCache")
...
func (r *Go) Test(src *Directory, opts ...GoTestOpts) *Container {
	assertNotNil("src", src)
...
func (r *Client) Go(opts ...GoOpts) *Go {
	...
	for i := len(opts) - 1; i >= 0; i-- {
		...
		// `modCache` optional argument
		if !querybuilder.IsZeroValue(opts[i].ModCache) {
			q = q.Arg("modCache", opts[i].ModCache)
		}
...
// Load a Sonar from its ID.
func (r *Client) LoadSonarFromID(id SonarID) *Sonar {
	q := r.query.Select("loadSonarFromID")
	q = q.Arg("id", id)
...
func (r *Client) Sonar() *Sonar {
	q := r.query.Select("sonar")
...
func (r *Sonar) WithGraphQLQuery(q *querybuilder.Selection) *Sonar {
	return &Sonar{
		query: q,
	}
}
...
// Run sonar scanner
func (r *Sonar) Analyze(ctx context.Context, src *Directory, token *Secret, opts ...SonarAnalyzeOpts) (string, error) {
	assertNotNil("src", src)
	assertNotNil("token", token)
	if r.analyze != nil {
		return *r.analyze, nil
	}
	q := r.query.Select("analyze")
	for i := len(opts) - 1; i >= 0; i-- {
		// `organization` optional argument
		if !querybuilder.IsZeroValue(opts[i].Organization) {
			q = q.Arg("organization", opts[i].Organization)
		}
		// `projectKey` optional argument
		if !querybuilder.IsZeroValue(opts[i].ProjectKey) {
			q = q.Arg("projectKey", opts[i].ProjectKey)
		}
		// `sources` optional argument
		if !querybuilder.IsZeroValue(opts[i].Sources) {
			q = q.Arg("sources", opts[i].Sources)
		}
	}
	q = q.Arg("src", src)
	q = q.Arg("token", token)
...
// A unique identifier for this Sonar.
func (r *Sonar) ID(ctx context.Context) (SonarID, error) {
	if r.id != nil {
		return *r.id, nil
	}
	q := r.query.Select("id")

	var response SonarID
...
// XXX_GraphQLType is an internal function. It returns the native GraphQL type name
func (r *Sonar) XXX_GraphQLType() string {
	return "Sonar"
}

// XXX_GraphQLIDType is an internal function. It returns the native GraphQL type name for the ID of this object
func (r *Sonar) XXX_GraphQLIDType() string {
	return "SonarID"
}

// XXX_GraphQLID is an internal function. It returns the underlying type ID
func (r *Sonar) XXX_GraphQLID(ctx context.Context) (string, error) {
	id, err := r.ID(ctx)
	if err != nil {
		return "", err
	}
...
func (r *Sonar) MarshalJSON() ([]byte, error) {
	id, err := r.ID(marshalCtx)
	if err != nil {
		return nil, err
	}
...
func (r *Sonar) UnmarshalJSON(bs []byte) error {
	var id string
	err := json.Unmarshal(bs, &id)
	if err != nil {
		return err
	}
	*r = *dag.LoadSonarFromID(SonarID(id))
	return nil
}
